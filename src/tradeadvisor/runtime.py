...
def build_service(settings: Settings | None = None) -> MarketDataService:
    ...
    return MarketDataService(adapter, store)
...
def load_frames(
    service: MarketDataService, symbol: str, entry_tf: str
    ...
    for role, tf in _role_timeframes(entry_tf).items():
        if tf not in by_tf:
            ...
            by_tf[tf] = service.get(symbol, tf, lookback_bars=lookback)
...
def load_backtest_frames(
    ...
    for role, tf in _role_timeframes(entry_tf).items():
        if tf not in by_tf:
            by_tf[tf] = service.get_range(symbol, tf, start_ms, end_ms, warmup_bars=warmup_bars)
        frames[role] = by_tf[tf]
    return frames
...
def analyze_symbol(
    ...
    frames = load_frames(service, symbol, entry_tf)
