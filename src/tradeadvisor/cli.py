...
def fetch(
    ...
    for tf in timeframes:
        df = service.get(symbol, tf, lookback_bars=bars)
...
def _print_recommendation(rec: Recommendation, raw: bool = False):
    typer.echo("")
    typer.echo(f"=== {rec.symbol}  [{rec.entry_timeframe}]  as of {rec.data_as_of:%Y-%m-%d %H:%M UTC} ===")
    ...
    if rec.position:
        ...
        typer.echo(f"Position  : {p.quantity:.6g} units (~{p.notional:,.2f} notional), "
                   f"risking {p.risk_amount:,.2f} = {p.risk_pct}% of {p.account_size:,.0f}")
    typer.echo("")
...
def analyze(
    ...
    raw: bool = typer.Option(False, help="also print detected S/R levels"),
):
...
def _print_report(rep: BacktestReport):
    typer.echo("")
    typer.echo(f"=== Backtest {rep.symbol} [{rep.entry_timeframe}] "
...
def backtest(
    ...
    frames = load_backtest_frames(service, symbol, tf, start_ms, end_ms)
